...
- **같은 형태의 대상을 여러 개 독립적으로** 만들 수 있습니다. `Quiz` 객체 8개는 각자 자기 문제를 갖습니다.
- **확장·유지보수가 쉽습니다.** 힌트 기능을 넣을 때 `Quiz`에 속성 하나만 추가하면 됐습니다.
- 트레이드오프: 아주 짧은 스크립트라면 함수만으로 충분하고 클래스는 다소 장황할 수 있습니다. 하지만 이 프로그램은 상태(퀴즈 목록·점수)를 계속 들고 여러 기능을 제공하므로, 클래스로 나눈 이점이 더 큽니다.
...
- **검색**: 키워드 검색을 추가하면 리스트 전체를 훑는 선형 탐색(O(n))이 됩니다. → 잦은 검색이면 인덱스나 DB가 유리.
